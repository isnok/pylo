...
def bisonToPython(bisonfileName, lexfileName, pyfileName, generateClasses=0):
    ...
    try:
        chapters = ([], [], [])
        reading = 0
        for line in rawBison.readlines():
            lstrip = line.strip()
            if not lstrip:
                continue
            elif lstrip == "%%":
                ...
                chapters[reading].append(line)
        prologue, rulesRaw, epilogue = chapters
    except:
        raise Exception(
    ...
    for line in prologueLines:
        ...
        if kwd == '%token':
            ...
            startTarget = args[0]
            print "start: %s %s" % (kwd, args)
        else:
            print "Unknown: %s %s" % (kwd, args)
    ...
    for line in rulesRaw:
        lstrip = line.strip()
        if lstrip == ';':
            rulesLines.append(rule)
            rule = ""
        elif lstrip.startswith("{"):
            continue # ditch c code
        else:
            rule = "%s %s" % (rule, lstrip)
    ...
    for target, options in rules:
        #if any(map(lambda x: any(map(lambda y: '"' in y, x)), options)):
            #print options
        tmp = []
        for t in options:
            joined = " ".join(t)
